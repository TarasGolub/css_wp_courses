<!--all_page_wrap-->
</div>
<!--main_container-->
</div>
<footer class="page_footer" id="page_footer">
    <div class="container">
        <div class="logo footer_logo">
            <a href="index.php" >Palo Alto</a>
        </div>
        <div class="menu footer_menu row">
            <nav>
                <div class="menu footer_menu_item col m-3 l-offset-2 l-2">
                    <a href="index.php" >Home</a>
                </div>
                <div class="menu footer_menu_item col m-3 l-2">
                    <a href="about.php" >About</a>
                </div>
                <div class="menu footer_menu_item col m-3 l-2">
                    <a href="archive.php" >Archive</a>
                </div>
                <div class="menu footer_menu_item col m-3 l-2">
                    <a href="contacts.php" >Contact</a>
                </div>
                <div class="search_icon_footer col m-1 hide-xs hide-s hide-m l-1">
                    <a href="search.php"><span class="search_icon">&#128269;</span></a>
                </div>
            </nav>
        </div>
        <div class="row">
            <div class="footer_info top_line col ">
                <p>
                    Lorem ipsum dolor sit amet, consectetur adipisicing elit. Assumenda cupiditate deleniti
                    facere facilis inventore maiores natus numquam tempora unde veniam.
                    dolor sit amet, consectetur adipisicing elit. Assumenda cupiditate deleniti
                    facere facilis inventore maiores natus
                </p>
            </div>
        </div>

        <div class="row">
            <div class="rights_reserved top_line col ">
                <p>
                    © 2015 - Palo Alto. All Rights Reserved. Designed & Developed
                    by <a href="http://example.com/" target="_blank">Pixel Buddha Team</a>
                </p>
            </div>
        </div>

        <div class="social_container">
            <div class="social">
                <div class="social_item">
                    <a href="http://facebook.com" target="_blank">
                        <img src="public/images/facebook.png" alt="facebook" title="facebook">
                    </a>
                </div>
                <div class="social_item">
                    <a href="http://twitter.com" target="_blank">
                        <img src="public/images/twitter.png" alt="twitter" title="twitter">
                    </a>
                </div>
                <div class="social_item">
                    <a href="http://instagram.com" target="_blank">
                        <img src="public/images/instagram.png" alt="instagram" title="instagram">
                    </a>
                </div>
                <div class="social_item">
                    <a href="http://pinterest.com" target="_blank">
                        <img src="public/images/pinterest.png" alt="pinterest" title="pinterest">
                    </a>
                </div>
            </div>
        </div>
    </div>
</footer>
<!--    <script src="public/js/jquery.maphilight.min.js"></script>-->
    <script src="http://ajax.googleapis.com/ajax/libs/jquery/3.1.0/jquery.min.js"></script>
    <script src="public/js/nprogress.js"></script>
    <script src="public/js/hideFoot.js"></script>
    <script type="text/javascript">
        NProgress.start(); // начать "загрузку"
        NProgress.done(); // заставить индикатор дойти до конца и пропасть
    </script>
<script src="public/js/slider.js"></script>
</body>